...
const sdk = new CoinbaseWalletSDK({
  appName: "Sunlit Keelstone (Built for Base)",
...
function safeTarget(v: string): `0x${string}` {
  const trimmed = v.trim();
  const fallback = session?.address;
  return addr(trimmed || (fallback ?? ""));
}
...
async function erc20Probe(token: string, holder: string) {
  const t = addr(token.trim());
  const h = safeTarget(holder);
  const c = pc();
...
function mount() {
  ...
  h1.textContent = "Sunlit Keelstone";
